...
void init_heap(){
    uint64_t* heap = mmap(NULL, HEAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    HEAP_START = heap;
...
void* my_malloc(size_t size){
    ...
    while(current < HEAP_START + (HEAP_SIZE / VAL_SIZE)){
        ...
        size_t cur_size = (cur_header / 2) * 2;
        if ((cur_header % 2 ==0) && (size <= cur_size)){
            size_t rounded = ((size +7) / 8) * 8;
            *current = rounded + 1;

            uint64_t remaining = cur_size - (rounded + VAL_SIZE);
